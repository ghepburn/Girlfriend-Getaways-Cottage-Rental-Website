...
export class Account extends Component {
	...
	async saveUser(user) {
		let savedUser = await UserManager.saveUser(user);
		if (savedUser.isAuthenticated) {
			this.props.authContext.changeUser(savedUser);
			this.props.notificationContext.sendNotification(NotificationManager.getSuccessfulSettingsChangeNotification());
		} else {
